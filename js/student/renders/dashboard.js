...
function renderMatrix() {
    ...
    // Use Helpers for Date/Label
    // If helpers are missing (race condition), fallback to defaults
...
function createMatrixCard(t) {
    ...
    if (t.checklist && t.checklist.length > 0) {
        ...
        progressHtml = `<div class="mt-3"><div class="flex justify-between items-end mb-1"><span class="text-[9px] font-bold text-muted uppercase tracking-wider">Progress</span><span class="text-[9px] font-bold text-primary">${done}/${total}</span></div><div class="h-1.5 w-full bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden"><div class="h-full bg-primary transition-all duration-500 ease-out" style="width:${pct}%"></div></div></div>`;
    }
    ...
    if (t.completed) {
        let stars = '';
        if (t.difficulty) for(let i=0; i<t.difficulty; i++) stars += '★';
        const timeSpent = t.actualTime ? `${t.actualTime}m` : '';
        if (stars || timeSpent) statsHtml = `<div class="mt-2 pt-2 border-t border-border flex justify-between items-center text-[10px] font-bold"><span class="text-yellow-500 tracking-widest">${stars}</span><span class="text-muted">${timeSpent} spent</span></div>`;
    }
...
function renderStudentPlanner() {
    ...
    else { const f = curr.getDate() - curr.getDay(); weekStart = new Date(curr.setDate(f)); }
...
function renderStream() {
    ...
    Object.values(groups).forEach(g => {
        ...
        g.items.forEach(t => {
            ...
            html += `<div onclick="openTaskDetails(${t.id})" class="bg-surface border border-border p-4 rounded-xl flex items-center justify-between shadow-sm hover:scale-[1.01] transition-all cursor-pointer group"><div class="flex items-center gap-4"><div class="w-1.5 h-12 rounded-full" style="background:${color}"></div><div><div class="font-bold text-base group-hover:text-primary transition-colors">${t.title}</div><div class="text-xs text-muted flex items-center gap-2"><span class="font-bold" style="color:${color}">${t.course}</span><span>•</span><span>${timeStr}</span></div></div></div><div class="text-right"><div class="font-mono text-sm font-bold">${t.est}m</div><button onclick="event.stopPropagation(); toggleComplete(${t.id})" class="mt-1 text-muted hover:text-green-500"><i class="fa-regular fa-square text-xl"></i></button></div></div>`;
        });
        html += `</div></div>`;
...
function renderKanban() {
    const cols = { todo: document.getElementById('kb-col-todo'), doing: document.getElementById('kb-col-doing'), done: document.getElementById('kb-col-done') };
    if(!cols.todo) return;
    ...
    const lists = { todo: cols.todo.querySelector('#kb-list-todo'), doing: cols.doing.querySelector('#kb-list-doing'), done: cols.done.querySelector('#kb-list-done') };
